...
interface PlasmaProps {
  ...
  /** fração da resolução real renderada (0.5 = metade). Mais baixo = mais leve. */
...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      const rect = containerEl.getBoundingClientRect();
      const width = Math.max(1, Math.floor(rect.width * rs));
      const height = Math.max(1, Math.floor(rect.height * rs));
  ...
  }, [color, speed, direction, scale, opacity, mouseInteractive, resolutionScale, maxFps]);
