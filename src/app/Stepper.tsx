...
export default function Stepper({
  ...
  renderStepIndicator,
  ...rest
}: StepperProps & { autoAdvanceMs?: number }) {
  ...
  // auto-avanço (loop) — cada step dura autoAdvanceMs
  useEffect(() => {
    if (!autoAdvanceMs) return;
    const id = setTimeout(() => {
      setDirection(1);
      updateStep(currentStep >= totalSteps ? 1 : currentStep + 1);
    }, autoAdvanceMs);
    return () => clearTimeout(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentStep, autoAdvanceMs, totalSteps]);
  ...
  return (
    <div className="flex w-full flex-col items-center" {...rest}>
      ...
      >
        ...
        {!isCompleted && (
          <div className={`px-6 pb-8 sm:px-8 ${footerClassName}`}>
            <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              ...
              <button
                ...
                className="duration-350 flex items-center justify-center rounded-full bg-accent py-1.5 px-3.5 font-medium tracking-tight text-white transition hover:opacity-90 active:opacity-80"
                {...nextButtonProps}
              >
